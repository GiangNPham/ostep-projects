...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const int math_mode) {
    ...
    switch(add_delta(c, key, nkey, math_mode, delta, temp, NULL)) {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
    ...
    case DELTA_ITEM_NOT_FOUND:
        ...
        } else c->thread->stats.div_misses++;
